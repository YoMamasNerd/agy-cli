...
prompt_name() {
    ...
    if [[ -n "$current" ]]; then
        ...
        read -r -p "Name [$current]: " reply || reply=""
    else
        echo "Leave it blank and a name will be generated for you."
        read -r -p "Name (optional): " reply || reply=""
    fi
...
exec "\$AGY_BIN" --remote-control${name_arg} "\$@"
EOF
...
do_login() {
    if [[ -s "$TOKEN_FILE" ]]; then
        echo "Notice: Already authenticated ($TOKEN_FILE); skipping login."
        return 0
    fi
    echo "--------------------------------------------------------------"
    ...
    local wd=$!
    trap 'echo' INT
    "$AGY_BIN" --remote-control >/dev/null || true
    trap - INT
    ...
    if [[ -s "$TOKEN_FILE" ]]; then
        ...
        echo "Warning: No auth token found at $TOKEN_FILE; the daemon may start unauthenticated." >&2
        echo "Re-run install to retry." >&2
    fi
}
...
show_usage() {
    echo "Usage: $0 [install [--no-auto-update] [--interval <interval>] | status | restart | uninstall]"
    echo ""
